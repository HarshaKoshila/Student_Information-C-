...
{
    public partial class ChrgSheetForm1 : Form
    {
        ...
        public ChrgSheetForm1()
        {
            InitializeComponent();
            FillGrid();
            btnUpdateChrg.Enabled = false;
            btnAddCsheet.Enabled = true;
        }

        public void FillGrid()
        {
            DataSet ds = ob.getChargeSheet();
            dgvChrgSheet.DataSource = ds.Tables["ChargeSheetOfKDU"].DefaultView;
        }

        public void FillGridFromSNO(String SNO)
        {
            DataSet ds = ob.getChargeSheetSNO(SNO);
            dgvChrgSheet.DataSource = ds.Tables["SNOChargeSheetOfKDU"].DefaultView;
        }

        public void FillGridFromIntake(String intake)
        {
            DataSet ds = ob.getChargeSheetFromIntake(intake);
            dgvChrgSheet.DataSource = ds.Tables["Intake_ChargeSheetOfKDU"].DefaultView;
        }
        ...
        {
            if ((txtIntkCsheet.Text).Equals("") || (txtNameCsheet.Text).Equals("") || (txtNoWitCsheet.Text).Equals("") || (txtOffCsheet.Text).Equals("") || (txtPunishCsheet.Text).Equals("") || (txtRnkCsheet.Text).Equals("") || (txtSNOCsheet.Text).Equals("") || (txtTrpCsheet.Text).Equals("") || (txtWitnCsheet.Text).Equals(""))
                return true;
            else
                return false;
        }
        ...
        private void btnAddCsheet_Click(object sender, EventArgs e)
        {
            if (isEmptyAll())
                MessageBox.Show("please fill in all required fields");
            else
            {
                String date1 = dtpDateCSheet.Value.Date.ToShortDateString();
                if (ob.addChargeSheet(txtSNOCsheet.Text, txtRnkCsheet.Text, txtTrpCsheet.Text, date1, txtOffCsheet.Text, txtNoWitCsheet.Text, txtWitnCsheet.Text, txtPunishCsheet.Text, imgHD))
                {
        ...
        private void txtSNOCsheet_Leave(object sender, EventArgs e)
        {

        }

        private void btnUploadCsheet_Click(object sender, EventArgs e)
        {
            imgHD = null;
        ...
        private void btnUpdateAtten_Click(object sender, EventArgs e)
        {
            ...
            {
                if (ob.updateChargeSheet(ID, txtRnkCsheet.Text, txtTrpCsheet.Text, dtpDateCSheet.Value.ToShortDateString(), txtOffCsheet.Text, txtNoWitCsheet.Text, txtWitnCsheet.Text, txtPunishCsheet.Text,imgHD))
                {
        ...
        private void dgvChrgSheet_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ID = dgvChrgSheet.Rows[e.RowIndex].Cells["ColumnID"].Value.ToString();
            }
            catch (ArgumentOutOfRangeException)
            {
                Console.WriteLine(e);
            }
            if (e.ColumnIndex == dgvChrgSheet.Columns["ColumnView"].Index)
            {
                FormImgViewer form = new FormImgViewer(ob.getChrgSheetImg(ID));
                form.ShowDialog();
            }


            if (e.ColumnIndex == dgvChrgSheet.Columns["ColumnDelete"].Index)
            {
                if (MessageBox.Show("Delete selected Job Post?", "Confirm!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                { 
                    if (ob.deleteChargeSheet(ID))
                    {
            ...
            if (e.ColumnIndex == dgvChrgSheet.Columns["ColumnSave"].Index)
            {
                string folderPath = "";
                ...
                {
                    SNO = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvSNO"].Value.ToString();
                    CheckvalidSNO(SNO);
                    ...
                    try
                    {
                        ...
                        for (int j = 4; j < dgvChrgSheet.Columns.Count; j++)
                        {
                            Paragraph p = new Paragraph(dgvChrgSheet.Columns[j].HeaderText + "     :" + dgvChrgSheet.Rows[e.RowIndex].Cells[j].Value);
        ...
        private void txtNameCsheet_MouseClick(object sender, MouseEventArgs e)
        {
            if (CheckvalidSNO(txtSNOCsheet.Text))
            {
                txtNameCsheet.Text = name;
                txtIntkCsheet.Text = intake;
            }
            else
                MessageBox.Show("Invalid SNO ");
        }

        private void dgvChrgSheet_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ID = dgvChrgSheet.Rows[e.RowIndex].Cells["ColumnID"].Value.ToString();
            txtSNOCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvSNO"].Value.ToString();
            if (CheckvalidSNO(txtSNOCsheet.Text))
            {
                txtNameCsheet.Text = name;
                txtIntkCsheet.Text = intake;
            }
            txtRnkCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvRank"].Value.ToString();
            txtTrpCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvTrp"].Value.ToString();
            dtpDateCSheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvDate"].Value.ToString();
            txtOffCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvOffence"].Value.ToString();
            txtNoWitCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvWitnessNo"].Value.ToString();
            txtWitnCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvWitness"].Value.ToString();
            txtPunishCsheet.Text = dgvChrgSheet.Rows[e.RowIndex].Cells["dgvPunish"].Value.ToString();

            btnUpdateChrg.Enabled = true;
            btnAddCsheet.Enabled = false;
        }

        private void btnSearchChrg_Click(object sender, EventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            PdfPTable pdfTable = new PdfPTable(dgvChrgSheet.ColumnCount - 3); //avoiding unnessary column
            ...
            for (int j = 3; j < dgvChrgSheet.Columns.Count; j++)
            {
                PdfPCell cell = new PdfPCell(new Phrase(dgvChrgSheet.Columns[j].HeaderText));
                cell.BackgroundColor = new iTextSharp.text.BaseColor(240, 240, 240);
                pdfTable.AddCell(cell);
            }

            //Adding DataRow
            foreach (DataGridViewRow row in dgvChrgSheet.Rows)
            {
                for (int j = 3; j < dgvChrgSheet.Columns.Count; j++)
                {
